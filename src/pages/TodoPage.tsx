...
export default function TodoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-white dark:from-blue-950/30 dark:to-gray-900 rounded-xl p-4 shadow-sm border border-blue-100 dark:border-blue-900/40 space-y-3">
        <div className="flex gap-2">
          <input value={content} onChange={e => setContent(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && !loading && handleCreate()}
            placeholder="添加待办..."
            className="flex-1 rounded-lg px-3 py-2 text-sm bg-white/80 focus:bg-white dark:bg-gray-800/80 dark:focus:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-300 transition" />
          <button onClick={() => fileInputRef.current?.click()}
            className="px-3 py-2 rounded-lg text-sm text-gray-600 dark:text-gray-300 bg-white/80 hover:bg-white dark:bg-gray-800/80 dark:hover:bg-gray-800 flex items-center gap-1 transition">
        ...
        {pendingFiles.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {pendingFiles.map((f, i) => (
              <div key={i} className="text-xs bg-gray-100 dark:bg-gray-800 dark:text-gray-300 px-2 py-1 rounded">{f.name}</div>
            ))}
          </div>
        )}
      </div>
...
function TodoList({ title, items, onToggle, onDelete, onDeleteImage }: {
  ...
  return (
    <div>
      <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{title}（{items.length}）</h2>
      <div className="space-y-2">
        {items.map(todo => (
          <div key={todo.id} className={`rounded-xl p-4 shadow-sm border transition ${
            todo.is_done
              ? 'bg-gradient-to-r from-gray-50 to-white dark:from-gray-800/40 dark:to-gray-900 border-gray-100 dark:border-gray-700/40'
              : 'bg-gradient-to-r from-blue-50 to-white dark:from-blue-950/30 dark:to-gray-900 border-blue-100 dark:border-blue-900/40'
          }`}>
            <div className="flex items-start gap-3">
              ...
              <span className={`flex-1 text-sm ${todo.is_done ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-800 dark:text-gray-100'}`}>
                {todo.content}
              </span>
