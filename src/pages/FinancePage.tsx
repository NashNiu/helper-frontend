    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {records.map((r) => (
          ...
          >
            {r.amount > 0 ? (
              <PlusIcon className="w-5 h-5 text-green-500 dark:text-green-400 flex-shrink-0" />
            ) : (
              <MinusIcon className="w-5 h-5 text-red-400 dark:text-red-400 flex-shrink-0" />
            )}
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span
                  className={`text-base font-semibold ${r.amount > 0 ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"}`}
                >
                  {r.amount > 0 ? "+" : ""}¥{Math.abs(r.amount).toFixed(2)}
                </span>
                <span className="text-xs bg-gray-100 dark:bg-gray-800 dark:text-gray-400 px-2 py-0.5 rounded-full text-gray-500">
                  {r.category}
                </span>
              </div>
