...
import { FinanceRecord } from '../api/finance';

const PIE_COLORS = ['#6366f1', '#f59e0b', '#10b981', '#ef4444', '#3b82f6', '#8b5cf6'];

interface Props { records: FinanceRecord[]; }

export default function FinanceCharts({ records }: Props) {
  if (records.length === 0) return <p className="text-sm text-gray-400 text-center py-8">暂无数据</p>;

  // 按日期聚合收支
  const byDay = records.reduce<Record<string, { income: number; expense: number }>>((acc, r) => {
    const day = new Date(r.happened_at).toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric' });
  ...
  const totalExpense = records.filter(r => r.amount < 0).reduce((s, r) => s + Math.abs(r.amount), 0);
