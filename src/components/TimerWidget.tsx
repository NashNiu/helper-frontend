...
export default function TimerWidget() {
  ...
  const timeColor = isDone ? 'text-emerald-600' : pomodoro?.phase === 'break' ? 'text-emerald-600' : 'text-indigo-600';

  return (
    <div className="fixed bottom-4 right-4 z-40 bg-white shadow-xl border rounded-2xl px-4 py-3 flex items-center gap-3 min-w-[230px]">
      <div className="text-2xl">{icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-xs text-gray-500 truncate">{titleText}</p>
      ...
      <div className="flex flex-col gap-1">
        ...
        {(isRunning || isPaused) && (
          <button onClick={reset} className="px-2 py-0.5 text-xs bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
            重置
          </button>
        )}
        {isDone && (
          <button onClick={clear} className="px-2 py-0.5 text-xs bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
            关闭
          </button>
        )}
      </div>
